	...
	def __init__(self, parent, controller):
		...
		def AllHardwareTesterButtons():
			...
			def KeyboardTester():
				...
				KEYBRDFUNCTLABEL = tk.Label(root, text="Put Stuff here 1234567890")
				KEYBRDFUNCTLABEL.grid()

				KEYBRDFUNCTENTRY = tk.Entry(root, bd=5)
				KEYBRDFUNCTENTRY.grid()

				KEYBRDBUTTON = tk.Button(root, width=60, height=2, text="Enter", command=lambda: show_entry_fields())
				KEYBRDBUTTON.grid(row=1, column=1, padx=5, pady=5)

				qwertyuiop =  "1234567890qwertyuiopasdfghjklzxcvbnm"
				root.mainloop()

				if KEYBRDBUTTON == qwertyuiop.lower():
